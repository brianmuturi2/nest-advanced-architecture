...
export class AlarmCreatedEventHandler implements IEventHandler<SerializedEventPayload<AlarmCreatedEvent>> {
    ...
    async handle(event: SerializedEventPayload<AlarmCreatedEvent>) {
        ...
        // In a realworld application, we would have to ensure that this operation is atomic
        // with creation of the alarm. Otherwise, we could end up with an alarm that is not reflected
